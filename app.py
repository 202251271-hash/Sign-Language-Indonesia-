...
CONF_THRES = 0.55
RF_MODEL_URL = os.getenv(
    "RF_MODEL_URL",
    "https://drive.google.com/uc?export=download&id=1-1rXiTj1kd3mtcUJ9ZJJ2eRu2xrTc7vV",
)
...
def _ensure_rf_model():
    ...
    try:
        print(f"Downloading RF model from: {RF_MODEL_URL}")
        urllib.request.urlretrieve(RF_MODEL_URL, RF_MODEL_PATH)
        print("RF model download complete.")
    except Exception as exc:
        raise RuntimeError(f"Gagal download rf_model.pkl: {exc}") from exc
